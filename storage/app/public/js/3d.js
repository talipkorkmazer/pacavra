...
let scene, camera, renderer, controls, hlight, directionalLight, light, light2, light3, light4;
...
function init(texture = null) {
  ...
  loader.load('/public/models/sofa_grey_fabric/scene.gltf', function(gltf) {
    ...
    if (mobileCheck()) {
      ...
      sofa.scale.set(4, 4, 4);
    }
    sofa.rotation.z += 2.75;
    sofa.rotation.x += 0.2;
    sofa.rotation.y += 0.2;

    sofaScene.traverse(async function(node) {
...
document.addEventListener('DOMContentLoaded', () => {
  console.log('ready!');
  init();
  const elements = document.getElementsByClassName("fabric-texture");
  for (let i = 0; i < elements.length; i++) {
    elements[i].addEventListener('click', changeTexture, false);
  }
});
